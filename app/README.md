...
- **격돌 · 관전 투표**: 격돌방이 이미 두 명으로 꽉 찬 뒤에 시간표/토론방 목록에서 누르면 참여가 아니라 **관전 모드**로 들어갑니다. 대화를 읽기 전용으로 보면서 A/B 중 한쪽에 1인 1표로 투표할 수 있고, 방 상단(참가자·관전자 모두)에 득표 비율이 얇은 게이지 바로 실시간 표시됩니다.
...
- **참가기록 어록은 AI가 요약**합니다 — 세션 종료 시 내가 실제로 한 발언들을 AI에게 보내 핵심 주장이 가장 잘 드러나는 한 문장으로 뽑습니다(실패 시 마지막 발언을 그대로 사용). 리허설룸도 연습이 끝나면(나가기) 동일하게 참가기록에 남습니다.
