...
- **진지한 대화**: 닉네임으로 입장 → 같은 주제에 실시간 매칭 → AI가 생성한 사전 브리핑(그 방에서 딱 한 번만 뜨고, 재접속/재입장해도 다시 안 뜨고 바로 대화 화면으로 이어짐) → 실시간 채팅(턴 교대, 인정 버튼, 손들기, 생각이 바뀜 선언, AI 조롱/인신공격 모더레이션) → 나가면 참가기록에 실제로 기록. 방을 만들거나 매칭에 신청하면 목록에 머물러 있지 않고 곧장 그 방(대기 중이면 대기 화면, 매칭됐으면 대화 화면)으로 들어갑니다.
