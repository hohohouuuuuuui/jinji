...
- **대표 발언자가 나가면 방도 같이 닫힙니다**: 방장이 아니어도, "나가기 · 참가기록 남기기"를 누른 사람이 대표 발언자(2번째 팀원이 아니라 seat_a/seat_b 본인)라면 그 세션은 계속될 수 없으니(그 자리는 다시 채워지지 않음) 방도 함께 종료 처리됩니다 — 예전엔 로컬 화면만 나가지고 실제 방은 계속 active로 남아, 상대는 답장을 영원히 기다리고 나간 사람 본인도 다음에 새로고침하면 그 방으로 도로 끌려들어갔습니다. 2번째 팀원이 나가는 경우는 팀 대표가 계속 있으니 방을 닫지 않습니다.
- (버그 수정) 자동생성 2번방("가장 후회하는 선택")이 실제로는 "1:1 대화"인데도 내부 `kind`가 `clash`로 잘못 설정되어 있었습니다 — 자리가 차면 평범한 대화방인데도 관전 투표 화면("A/B에게 투표")으로 빠지는, 맥락에 안 맞는 화면이 뜨는 버그였습니다. `chat`으로 고쳤습니다.
