...
- **격돌 · 관전 투표**: 격돌방이 이미 두 명으로 꽉 찬 뒤에 시간표/토론방 목록에서 누르면 참여가 아니라 **관전 모드**로 들어갑니다. 대화를 읽기 전용으로 보면서 A/B 중 한쪽에 1인 1표로 투표할 수 있고, 방 상단(참가자·관전자 모두)에 득표 비율이 얇은 게이지 바로 실시간 표시됩니다. 사용자가 만든 격돌방뿐 아니라 **자동생성 2번·3번방**도 꽉 차면 "마감" 대신 "관전하기"로 바뀌어 같은 방식으로 관전할 수 있습니다(1번방은 순수 1:1이라 관전이 없습니다).
