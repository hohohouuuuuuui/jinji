...
- **격돌 방 종류**: 격돌방(사용자가 만든 방 + 자동생성 2번·3번방)은 두 명이 차면 시간표/토론방 목록에서 "마감"으로 빠집니다. 관전 모드(읽기 전용 + A/B 투표 + 실시간 득표 게이지) 자체는 코드에 구현돼 있지만(`ClashSpectatorTab`, `useSpectate`, `useVotes`, `VoteGauge`), 현재는 목록에서 들어갈 진입점을 빼둔 상태입니다.
